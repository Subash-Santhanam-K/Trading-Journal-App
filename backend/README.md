...
### 1. Booting the Backend API (Local Development)
```bash
cd backend
npm install
```
...
### 2. Booting the Backend API (Docker / Production)
Run the backend seamlessly using Docker:
```bash
cd backend
...
Or using Docker Compose mapping environments natively:
```bash
docker-compose up --build -d
```

### 3. Booting the Interactive Dashboard
```bash
cd ../frontend
npm install
npm run dev
